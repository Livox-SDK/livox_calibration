...
struct LidarEdgeCalibFactor {
  LidarEdgeCalibFactor(Eigen::Vector3d curr_point_, Eigen::Vector3d last_point_a_,
  ...
  template<typename T>
  bool operator()(const T *q, const T *t, const T *main_q_aux_buf, const T *main_t_aux_buf, T *residual) const {
    ...
    Eigen::Quaternion<T> main_q_aux{main_q_aux_buf[3], main_q_aux_buf[0], main_q_aux_buf[1], main_q_aux_buf[2]};
    Eigen::Matrix<T, 3, 1> main_t_aux{main_t_aux_buf[0], main_t_aux_buf[1], main_t_aux_buf[2]};

    Eigen::Matrix<T, 3, 1> lp_main;
    lp_main = main_q_aux * cp + main_t_aux;

    Eigen::Matrix<T, 3, 1> lp;
    lp = q_last_curr * lp_main + t_last_curr;
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_,
                                     ...
                                     const double s_) {
    return (new ceres::AutoDiffCostFunction<LidarEdgeCalibFactor, 3, 4, 3, 4, 3>(
        new LidarEdgeCalibFactor(curr_point_, last_point_a_, last_point_b_, s_)));
  }
...
struct LidarPlaneCalibFactor {
  LidarPlaneCalibFactor(Eigen::Vector3d curr_point_, Eigen::Vector3d last_point_j_,
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_,
                                     ...
                                     const double s_) {
    return (new ceres::AutoDiffCostFunction<LidarPlaneCalibFactor, 1, 4, 3, 4, 3>(
        new LidarPlaneCalibFactor(curr_point_, last_point_j_, last_point_l_,
                             last_point_m_, s_)));
  }
...
struct LidarPlaneNormCalibFactor {
  LidarPlaneNormCalibFactor(Eigen::Vector3d curr_point_,
  ...
  bool operator()(const T *q, const T *t, const T *main_q_aux_buf, const T *main_t_aux_buf, T *residual) const {
    ...
    point_w = q_w_curr * lp_main + t_w_curr;
  ...
  static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_,
    ...
    return (new ceres::AutoDiffCostFunction<LidarPlaneNormCalibFactor, 1, 4, 3, 4, 3>(
        new LidarPlaneNormCalibFactor(curr_point_, plane_unit_norm_, negative_OA_dot_norm_)));
  }
