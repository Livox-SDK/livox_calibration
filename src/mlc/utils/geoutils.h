#ifndef ALOAM_MLC_UTILS_GEOUTILS_H
#define ALOAM_MLC_UTILS_GEOUTILS_H
...
template<typename Scalar_t>
Eigen::Matrix<Scalar_t, 3, 1> TransformPoint(const Eigen::Matrix<Scalar_t, 4, 4> &H,
                                             const Eigen::Matrix<Scalar_t, 3, 1> &p) {

  Eigen::Matrix<Scalar_t, 3, 3> R = H.template topLeftCorner<3, 3>();
  Eigen::Matrix<Scalar_t, 3, 1> T = H.template rightCols<1>(). template head<3>();
...
template <class T>
Eigen::Quaternion<T> GetAxisRotateQuaternion(const T angle, const int axis) {
...
inline Eigen::Quaterniond ExpAdd(const Eigen::Quaterniond& q, const Eigen::Vector3d& dr) {
  Eigen::Quaterniond dq;
  dq.vec() = dr * 0.5;
  dq.w() = std::sqrt(1.0 - dq.vec().squaredNorm());
  return q * dq;
};

inline Eigen::Quaterniond AddYpr(const Eigen::Quaterniond& q, const Eigen::Vector3d& dr) {
  Eigen::Quaterniond dq;
  dq = GetAxisRotateQuaternion(dr(2), 2) * GetAxisRotateQuaternion(dr(1), 1) * GetAxisRotateQuaternion(dr(0), 0);
  return q * dq;
};

inline Eigen::Quaterniond AddPry(const Eigen::Quaterniond& q, const Eigen::Vector3d& dr) {
  Eigen::Quaterniond dq;
  dq = GetAxisRotateQuaternion(dr(1), 1) * GetAxisRotateQuaternion(dr(0), 0) * GetAxisRotateQuaternion(dr(2), 2);
  return q * dq;
};


template<typename Derived>
Eigen::Quaternion<typename Derived::Scalar> pry_to_quaternion(const Eigen::DenseBase<Derived> &pry) {
  EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
  EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
  EIGEN_STATIC_ASSERT(Derived::ColsAtCompileTime == 1, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);

  Eigen::Quaternion<typename Derived::Scalar> pry_q_ypr(-0.5,0.5,0.5,0.5);

  return pry_q_ypr * ypr_to_quaternion(pry) * pry_q_ypr.conjugate();
}

template<typename Scalar_t>
Eigen::Matrix<Scalar_t, 3, 1> quaternion_to_pry(const Eigen::Quaternion<Scalar_t> &q_) {
  Eigen::Quaternion<Scalar_t> q = q_.normalized();

  Eigen::Quaternion<Scalar_t> pry_q_ypr(-0.5,0.5,0.5,0.5);
  q = pry_q_ypr.conjugate() * q * pry_q_ypr;
